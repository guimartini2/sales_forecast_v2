...
if projection_type == 'Units':
    picks = [c for c in cols if re.search(r'unit', c, re.IGNORECASE)]
    y_col = picks[0] if picks else cols[1]
    forecast_label = 'Sell-Out Units'
    y_label = 'Units'
else:
    picks = [c for c in cols if re.search(r'sales', c, re.IGNORECASE)]
    y_col = picks[0] if picks else cols[1]
