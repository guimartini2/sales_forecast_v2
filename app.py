...
import pandas as pd
import numpy as np
# Try to import Prophet and set a flag
...
# Dynamically build model choices based on availability
model_options = []
if PROPHET_INSTALLED:
    model_options.append("Prophet")
model_options += ["ARIMA", "XGBoost"]
...
if st.sidebar.button("Run Forecast"):
    # Check for Prophet availability if selected
    if model_choice == "Prophet" and not PROPHET_INSTALLED:
        st.error("Prophet library not found. Install with: pip install prophet (or pip install fbprophet)")
    # Validate required uploads
    elif not history_file or not inventory_file:
