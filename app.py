...
for i, row in df_fc.iterrows():
    ...
    df_fc.at[i,'On_Hand_Begin'] = int(prev_on)
    prev_on = prev_on + Q - D

# Weeks of Cover constant
df_fc['Weeks_Of_Cover'] = woc_target

# Merge Amazon upstream
if upstream_path:
...
if 'Amazon_Sellout_Forecast' in df_fc.columns:
    df_fc['Sell-Out Units'] = df_fc['Amazon_Sellout_Forecast']
else:
    df_fc['Sell-Out Units'] = df_fc[forecast_label]
if forecast_label in df_fc.columns:
    df_fc.drop(columns=[forecast_label], inplace=True)
...
# Define metrics and filter to existing columns
...
if PLOTLY_INSTALLED:
    ...
    # Apply styling only if traces exist
    for name, style in [('Sell-Out Units', dict(color=AMZ_ORANGE, dash='dash')),
                        ('Safety_Stock', dict(color=AMZ_BLUE, dash='dot')),
                        ('Replenishment', dict(color=AMZ_ORANGE, dash='solid')),
                        ('Amazon_Sellout_Forecast', dict(color=AMZ_BLUE, dash='solid'))]:
        if name in metrics:
            fig.update_traces(selector=dict(name=name), line=style)
    st.plotly_chart(fig, use_container_width=True)
else:
    # Basic chart
    basic_df = df_fc.set_index('Week_Start')[metrics]
